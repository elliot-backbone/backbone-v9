  ...
  return (
    <SectionWrapper label="Core Metrics">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        {runway && (
          <div>
            <div className="text-xs text-gray-500 uppercase tracking-wide">
              Runway <span className="text-gray-400 text-[10px]">(derived)</span>
            </div>
            <div className={`text-lg font-semibold ${runway < 6 ? 'text-red-600' : runway < 12 ? 'text-amber-600' : ''}`}>
              {runway} mo
            </div>
          </div>
        )}
        {data.employees && (
          <div>
            <div className="text-xs text-gray-500 uppercase tracking-wide">Headcount</div>
        ...
        {data.mrr && (
          <div>
            <div className="text-xs text-gray-500 uppercase tracking-wide">MRR</div>
